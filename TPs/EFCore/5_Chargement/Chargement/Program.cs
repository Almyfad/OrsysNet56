...
static void DonnesLocales()
{
	...
	// Charger les données du DbSet en mémoire
	 context.Voyages.Where(v => v.CategorieId == 3).ToList();
	 context.Voyages.Where(v => v.CategorieId == 4).ToList();
	...
	{
		Console.WriteLine($"> {v.CodeVoyage} {v.Destination} {v.Prix}  {v.CategorieId}");
	}
...
{
    using var context = new VoyagesDbContext();
    List<Voyage> voyages = context.Voyages.Where(it => it.CategorieId == 3)
